...
export default function CarWashScreen() {
  const [activeTab, setActiveTab] = useState<'ticket' | 'home' | 'settings'>(
    'home',
  );
      ...
      get(child(dbRef, `users/${uid}`))
        .then(snapshot => {
          if (snapshot.exists()) {
            ...
            Alert.alert('There is no user yet');
          }
        })
        .catch(error => {
          Alert.alert(error);
  ...
  return (
    <View style={styles.container}>
      <HeaderSection styles={styles} fullName={fullName} photoUrl={photoUrl} />
...
const styles = StyleSheet.create({
  ...
  discountBg1: {
    ...
    width: 197,
    height: 90,
    backgroundColor: 'rgba(255,163,26,0.17)',
    borderRadius: 10,
  },
  discountBg2: {
    position: 'absolute',
